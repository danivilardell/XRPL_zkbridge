...
// *******************************************************
// ********** Lock XRP for a Bridge transfer  ************
// *******************************************************

async function lockXRP() {    
  results  = "Connecting to the selected ledger.\n"
  standbyResultField.value = results
  let net = getNet()
  results = 'Connecting to ' + getNet() + '....'
...
// -------------------------------------------------------- Prepare transaction
  const prepared = await client.autofill({
    ...
    "XChainBridge": {
      "LockingChainDoor": operational_wallet.address,
      "LockingChainIssue": {
        "currency": "XRP"
      },
      "IssuingChainDoor": operationalDestinationField.value,
      "IssuingChainIssue": {
        "currency": "XRP"
      }
    },
    "Amount": xrpl.xrpToDrops(standbyAmountField.value),
    "XChainClaimID": "13f"
  })
...
// -------------------------------------------------------- Submit signed blob
  ...
  operationalBalanceField.value = (await client.getXrpBalance(operational_wallet.address))                 
  client.disconnect()    
} // End of sendXRPBridge()

// *******************************************************
// ********* Generate signature validity proof  **********
// *******************************************************
